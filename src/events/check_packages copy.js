  ...
  if (values.length > 0) {
    for (var i = 0; i < values.length; i++) {
      ...
      if (getCorreiosInfo.origem == undefined) {
        if (getDataInfo[i].status != getCorreiosInfo.status) {
          let result = await Pacote.updateOne(
            ...
            { $set: { status: getCorreiosInfo.status, origem: getCorreiosInfo.origem, destino: getCorreiosInfo.destino, local: getCorreiosInfo.local},
            }
          );

          if (result.nModified > 0) {
            client.users.fetch(getDataInfo[i].userId, false).then((user) => {
              if (getDataInfo[i]?.apelido) {
                user.send(atualizacaoOrigemUndefined);
              } else {
                user.send(atualizacaoOrigemUndefinedByCode);
              }
            });
          }
        }
        ...
        if (getDataInfo[i].local != getCorreiosInfo.local) {
          ...
          if (result.nModified > 0) {
            client.users.fetch(getDataInfo[i].userId, false).then((user) => {
              if (getDataInfo[i].apelido == undefined) {
                user.send(atualizacaoOrigemUndefinedByCode);
              } else {
                user.send(atualizacaoOrigemUndefined);
              }
      ...
      if (getCorreiosInfo.local == undefined) {
        if (getDataInfo[i].status != getCorreiosInfo.status) {
          ...
          if (result.nModified > 0) {
            client.users.fetch(getDataInfo[i].userId, false).then((user) => {
              if (getDataInfo[i].apelido == undefined) {
                user.send(atualizacaoLocalUndefinedByCode);
              } else {
      ...
      let atualizacaoOrigemUndefined = new MessageEmbed()
        .setColor("GOLD")
        .setTitle(
          `Encontrei uma atualização no seu pedido ${getDataInfo[i].apelido}`
        )
        .setDescription(
      ...
      let atualizacaoOrigemUndefinedByCode = new MessageEmbed()
        .setColor("GOLD")
        .setTitle(
      ...
      let atualizacaoLocalUndefinedByCode = new MessageEmbed()
        .setColor("GOLD")
        .setTitle(
          `Encontrei uma atualização no seu pedido ${getDataInfo[i].trackCode}`
        )
